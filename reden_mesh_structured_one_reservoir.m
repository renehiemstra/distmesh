...
% mesh elements
n = [length(x1); length(x2); length(x5); length(x6);];
T = [T1; T2+n(1); T5+sum(n(1:2)); T6+sum(n(1:3))];
...
mesh.physobj(5).name = 'reservoir-top';
mesh.physobj(5).dim = 1;
mesh.physobj(5).tag = 5;

mesh.physobj(6).name = 'reservoir-left';
mesh.physobj(6).dim = 1;
mesh.physobj(6).tag = 6;

mesh.physobj(7).name = 'channel';
...
% mesh points
mesh.entity(1).dim = 0;
mesh.entity(1).data = [0 0 0];
mesh.entity(2).dim = 0;
mesh.entity(2).data = [o+xa 0 0];
mesh.entity(3).dim = 0;
mesh.entity(3).data = [o+xa ya 0];
mesh.entity(4).dim = 0;
mesh.entity(4).data = [o ya 0];
mesh.entity(5).dim = 0;
mesh.entity(5).data = [o 0.5 0];
mesh.entity(6).dim = 0;
mesh.entity(6).data = [0 0.5 0];

% mesh lines
tol = 1e-4;
mesh.entity(7).dim = 1;
mesh.entity(7).data = extractboundary(mesh, @(x) abs(x)<tol, @(y) y<0.5+tol & y>-tol);
mesh.entity(7).physobj = 'inlet';
mesh.entity(8).dim = 1;
mesh.entity(8).data = extractboundary(mesh, @(x) x<o+xa+tol & x>-tol, @(y) abs(y)<tol);
mesh.entity(8).physobj = 'symmetry';
mesh.entity(9).dim = 1;
mesh.entity(9).data = extractboundary(mesh, @(x) abs(x-(o+xa))<tol, @(y) y<ya+tol & y>-tol);
mesh.entity(9).physobj = 'outlet';
mesh.entity(10).dim = 1;
mesh.entity(10).data = extractboundary(mesh, @(x) x<o+xa+tol & x>o-tol, @(y) abs(y-ya)<tol);
mesh.entity(10).physobj = 'reservoir-top';
mesh.entity(11).dim = 1;
mesh.entity(11).data = extractboundary(mesh, @(x) abs(x-o)<tol, @(y) y<ya+tol & y>0.5-tol);
mesh.entity(11).physobj = 'reservoir-left';
mesh.entity(12).dim = 1;
mesh.entity(12).data = extractboundary(mesh, @(x) x<o+tol & x>0-tol, @(y) abs(y-0.5)<tol);
mesh.entity(12).physobj = 'channel';

% mesh surfaces
mesh.entity(13).dim = 2;
mesh.entity(13).data = T;
mesh.entity(13).bctag = 1:6;
mesh.entity(13).physobj = 'volume';
